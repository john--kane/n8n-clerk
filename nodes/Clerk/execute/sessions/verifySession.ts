...
export async function verifySession(ef: IExecuteFunctions) {
	try {
		...
		const token = ef.getNodeParameter('token', 0) as string | undefined;

		if (!sessionId || !token) {
			throw new NodeOperationError(ef.getNode(), 'Session identifier and token are required');
		}
		...
		const response = await client.sessions.verifySession(sessionId, token);
		return Ok(response);
	} catch (error) {
